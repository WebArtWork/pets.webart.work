...
export type LeadFormKind = 'owner' | 'specialist' | 'shelter' | 'business';

const KIND_LABELS: Record<LeadFormKind, string> = {
	owner: 'Заявка власника тварини',
	specialist: 'Заявка спеціаліста',
	shelter: 'Заявка притулку',
	business: 'Заявка бізнесу',
};

/** Inline lead-capture form used on the /for-* landing pages. Sends to the Pets Telegram contact channel. */
@Component({
...
export class LeadFormComponent {
	...
	protected async submit(form: NgForm): Promise<void> {
		...
		const lines = [
			`Pets lead: ${KIND_LABELS[this.kind()]}`,
			`Ім'я: ${this.name()}`,
